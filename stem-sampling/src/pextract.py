...
# Dummy function for Python2 compatibility
def unicode(s):
    return s
...
def string_to_varstring(string, vars):
    ...
    while idx < len(string):
        if string[idx] == u'[':
            ...
            s.append(string[idx])
            idx += 1
    return u''.join([str(x) for x in s])
...
def main(data_file, res_file):
    lines = [l.strip() for l in open(data_file)]
    tables = []
    thistable = []
    thesetags = []
    for l in lines:
