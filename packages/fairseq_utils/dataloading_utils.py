...
def get_initial_generation_frame(language, aug_pool_size): # concatenation of test file and augmentation file.
    ...
    augmentation_frame = pd.read_csv(f"{HALL_DATA_PATH}/{language}-train-low-hall-{aug_pool_size}", header=None, names=["src", "tgt" ,"tag", "invariant", "test_foreign_key"], sep='\t')
    assert len(augmentation_frame) == aug_pool_size 
